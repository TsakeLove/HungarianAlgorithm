...
void output(point** array, int size){
    temp+=1;
    cout<< "STEP #"<<temp<<endl;
    for (int i = 0; i < size; i++) {
        for (int l = 0; l < size; l++){
...
int min_in_row(int* array, int size){
    int min = array[0];
    for (int i = 0; i < size; i++) {
...
int min_in_column(int** array, int column_index, int size){
...
void delete_min_from_rows(int** array, int size){
    for (int i = 0; i < size; i++) {
        int min = min_in_row(array[i], size);
        for (int l = 0; l < size; l++) {
...
void delete_min_from_columns(int** array, int size){
    for (int l = 0; l < size; l++) {
        int min = min_in_column(array, l, size);
        for (int i = 0; i < size; i++) {
            array[i][l] = array[i][l] - min;
        }
    }
...
bool choose_zero(point** new_array, int size, int column, int start_column){
    ...
    for (int l = 0; l < size; l++) {
        if (new_array[l][column].number == 0) {
            ...
            if (choose_zero(new_array, size, column_index, start_column)) {
...
void choose_zero_permuted(point** new_array, int size, int i){
    bool flag = false;
    for (int l = 0; l < size; l++) {
    ...
    if (!flag){
        for (int l = 0; l < size; l++) {
            if (new_array[l][i].number == 0) {
                int column_index = find_used_zero(new_array, size, l);
                if (choose_zero(new_array, size, column_index, i)) {
                    new_array[l][i].used = 1;
...
point** set_zeros(int** array, int size){
    ...
    for (int i = 0; i < size; i++) {
        choose_zero_permuted(new_array, size, i);
    }
    return new_array;
}
...
point** set_zeros(point** array, int size){
    for (int i = 0; i < size; i++) {
        choose_zero_permuted(array, size, i);
    }
    return array;
}
...
void work(int** array, int size){
    ...
    int answer = find_sum(initial_array, final_array, size);
